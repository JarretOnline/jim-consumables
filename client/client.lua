...
RegisterNetEvent('jim-consumables:client:Drink2Bottle', function(itemName) ---use with jim-consumables to configure thirst levels

    ExecuteCommand( "e drink" )
    -->
    QBCore.Functions.Progressbar("drink_something", "Drinking", math.random(4000,9000), false, true, {
    ...
    -->
    TriggerServerEvent("jim-consumables:server:addThirst", QBCore.Functions.GetPlayerData().metadata["thirst"] + math.random(5,10))
    Wait(3000)
    ExecuteCommand( "e c" )
    ExecuteCommand( "pf" )
...
RegisterNetEvent('jim-consumables:Consume', function(itemName)
	...
	if consuming then
        ...
        if Config.UseProgbar then
            ...
            exports['okokNotify']:Alert("ITEM", string, 4000, 'success')
        end
    ...
    --Notification
    --QBCore.Functions.Notify(string, 'success', 9000)
    exports['okokNotify']:Alert("ITEM", string, 4000, 'success')
        ...
        if Config.Consumables[itemName].stats then
			...
			if stats.canOD then
				...
				if drugCount >= 3 then
					...
					SetEntityHealth(PlayerPedId(), GetEntityHealth(PlayerPedId()) - math.random(5,15))
					if drugCount >= 4 then
